...
int selectLease(char sigun_gu[20], char rent_type[20], int minDepoist, int maxDeposit, int minPyeong, int maxPyeong)
{
    ...
    // sql 실행
    char* sql = "SELECT id, b_name, b_dongnum, floor, sigun_gu, rent_type, deposit, pyeong FROM BUILDINGS WHERE sigun_gu = ? AND rent_type = ? AND deposit BETWEEN ? AND ? AND pyeong BETWEEN ? AND ?";
    ...
    if (bindNumParameters(stmt, minDepoist, parameterIndex++) != 0) {
        sqlite3_finalize(stmt);
        sqlite3_close(db);
        return 1;
    }
    ...
    printf("-----------------------------------------------------------------------------------------------\n");
    printf(" %3s    %15s        %5s    %3s    %5s      %5s    %5s     %5s \n",
    ...
    // SQL 문 실행 및 결과 처리
    while (sqlite3_step(stmt) == SQLITE_ROW && result <= 10) { // SQL 문 실행 및 결과가 있는 경우 (행 데이터 하나 가져옴)
        int i;
